...
import rcprg_smach.bring_goods_tasker
...
class MyTaskER(TaskER):
    def __init__(self, da_state_name, da_name):
        ...
        self.sis = smach_ros.IntrospectionServer(
            str("/"+self.name+"smach_view_server"), self, self.name)
        self.sis.start()
        places_xml_filename = self.get_parameter(
            '/kb_places_xml').get_parameter_value().string_value
        self.sim_mode = str(self.get_parameter(
            '/sim_mode').get_parameter_value().string_value)
        ...
        self.my_fsm = rcprg_smach.bring_goods_tasker.BringGoods(
    ...
    def update_task_tf(self):
        ...
        imp.reload(rcprg_smach.bring_goods_tasker)
        print("SWAPPING")
        self.swap_state('ExecFSM', rcprg_smach.bring_goods_tasker.BringGoods(
            self.sim_mode, self.conversation_interface, self.kb_places))
        self.sis.start()
