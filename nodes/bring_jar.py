...
class MainSM(Node, smach_rcprg.StateMachine):
    def __init__(self):
        Node.__init__(self, 'main_sm')
        smach_rcprg.StateMachine.__init__(
            self, outcomes=['Finished', 'shutdown'])

        # Adjust parameters for ROS2
        ...
        self.get_logger().info(
            f'Reading KB for places from file "{places_xml_filename}"')
        ...
        with self:
            smach_rcprg.StateMachine.add('BringJar',
                                         rcprg_smach.bring_jar.BringJar(
