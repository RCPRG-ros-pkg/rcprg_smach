...
def ptf_csp(ptf_id):
    ...
    destination_pose = csp_handler.get_robot_pose(sim_mode, kb_places, guide_destination)
    if destination_pose == 'is-volumetric':
        destination_pose = csp_handler.get_robot_pose(sim_mode, kb_places, guide_destination, human_name)
    if ptf_id[0] == "scheduleParams":
        ...
        req = ptf_id[1]
        return [flag, csp_handler.get_cost_conditions_gh(req, human_approach_pose, human_name)]# CostConditionsResponse(cost_per_sec=1, cost_to_complete=1)]
class Greeted():
    def __init__(self):
        self.value = False
...
class MyTaskER(TaskER):
    def __init__(self,da_state_name, da_name):
        ...
        self.my_fsm = rcprg_smach.guide_human.GuideHuman(self.sim_mode, self.conversation_interface, kb_places, self.greeted)
        human_name = None
        guide_destination = None
    ...
    def get_suspension_tf(self,susp_data):
        ...
        if self.greeted.get() == True:
            ...
            mod = imp.load_source("SuspGH",ts_path+"/src/rcprg_smach/suspend_gh.py")
            self.sis.stop()
            self.sis.clear()
            self.swap_state(u'ExeSuspension', mod.SuspGH(self.sim_mode, self.conversation_interface, kb_places))
            self.sis.start()
    ...
    def update_task_tf(self):
        ...
        print "SWAPPING"
        self.swap_state('ExecFSM', rcprg_smach.guide_human.GuideHuman(self.sim_mode, self.conversation_interface, kb_places,self.greeted))
        self.sis.start()
    ...
    def initialise_tf(self):
        ...
        if guide_destination is None:
            raise Exception('Argument "guide_destination" is missing in argv: ' + str(sys.argv))        
