...
def ptf_csp(ptf_id):
    ...
    global base_frame
    node = rclpy.create_node('temporary_node_for_param_get')
    human_posture = node.get_parameter("/"+human_name+"/actor_posture").value
    if human_posture is None:
        human_posture = "stand"
    flag = None
    ...
    robot_pose_updater.update_pose(base_frame, False, is_human)
...
class MyTaskER(TaskER):
    def __init__(self, da_state_name, da_name):
        ...
        places_xml_filename = self.node.get_parameter('kb_places_xml').value
        self.sim_mode = self.node.get_parameter('sim_mode').value
        assert self.sim_mode in ['sim', 'gazebo', 'real']
        sim_mode = self.sim_mode
        self.node.get_logger().info('Reading KB for places from file "%s"' %
                                    places_xml_filename)
        ...
        self.my_fsm = rcprg_smach.human_fell.HumanFell(
            self.sim_mode, self.conversation_interface, self.kb_places)
    ...
    def get_suspension_tf(self, susp_data):
        ...
        if fsm_executable == None:
            print(
                "harmoniser did not specified executable for suspension behaviour, terminating task")
    ...
    def update_task_tf(self):
        ...
        print("SWAPPING")
        self.swap_state('ExecFSM', rcprg_smach.human_fell.HumanFell(
            self.sim_mode, self.conversation_interface, self.kb_places))
        self.sis.start()
