...
from nav2_msgs.srv import GetParameters
import time
from nav2_statuses import Nav2Client
from geometry_msgs.msg import PoseStamped
from action_msgs.msg import GoalStatus

NAVIGATION_MAX_TIME_S = 100

class RememberCurrentPose(Node, TaskER.BlockingState):
    def __init__(self):
        super().__init__('remember_current_pose')

        # In ROS 2, parameters can be declared and used to set up configuration such as 'sim_mode'
        self.declare_parameter('sim_mode', 'real')  # Example default value 'real'
        sim_mode = self.get_parameter('sim_mode').get_parameter_value().string_value
        assert sim_mode in ['sim', 'gazebo', 'real']
        self.sim_mode = sim_mode

        # Set up a subscriber if running in 'gazebo' or 'real' mode
    ...
    def execute(self):
        self.get_logger().info('Executing state: RememberCurrentPose')

        if self.sim_mode == 'sim':
            # Simulation mode logic would go here
            pass
        else:
            for _ in range(10):
                ...
                with self.lock:
                    if self.current_pose:
                        # Logic to handle the current pose would go here
                        # For example, store it, or pass it as output from a state machine
                        break
            else:
                # If the pose is not received, handle the error
                self.get_logger().error('No pose received within the allotted time')

    def transition_function(self, userdata):
        # self.get_logger().info(f'Executing state: {self.__class__.____name__}')
        print("EXECUTING STATE: ", type(self).__name__)

        if self.sim_mode == 'sim':
            # Replace 'makePose' with your actual logic to create a Pose object
            pose = Pose()
            userdata.current_pose = PoseDescription(pose=pose)
            ...
            for i in range(10):
                ...
                if self.current_pose is not None:
                    pose_valid = True
                    userdata.current_pose = PoseDescription(
...
class UnderstandGoal(TaskER.BlockingState):
    ...
    def transition_function(self, userdata):
        # self.get_logger().info('{}: Executing state: {}'.format(self.get_name(), type(self).__name__))
        print("EXECUTING STATE: ", type(self).__name__)
        ...
        if self.sim_mode == 'sim':
            ...
            if not pose_valid:
                ...
                pt_start = (
                    current_pose.parameters['pose'].position.x, current_pose.parameters['pose'].position.y)
                print("pt_start: ", pt_start)
                try:
                    pl = self.kb_places.getPlaceByName(place_name, mc_name)
                except:
                    userdata.move_goal = PoseDescription(
                        {'pose': pose, 'place_name': place_name})
                    print('UnderstandGoal place_name is not valid')
                    return 'error'
...
#
                if pl.getType() == 'point':
                    if pl.isDestinationHuman():
                        human_pose = yaml.load(
                            rospy.get_param(place_name+'/pose'))
                        pt_dest = human_pose['x'], human_pose['y']
                        norm = math.cos(human_pose['theta']), math.sin(
                            human_pose['theta'])
                    else:
                        pt_dest = pl.getPt()
                        norm = pl.getN()
                    ...
                    print('pt: {}, pt_dest: {}, norm: {}, angle_dest: {}'.format(
                        pt, pt_dest, norm, angle_dest))
                elif pl.getType() == 'volumetric':
                    pt_dest = self.kb_places.getClosestPointOfPlace(
                        pt_start, pl.getId(), mc_name, dbg_output_path='/tmp/')
                    angle_dest = 0.0
                else:
...
class SetHeight(TaskER.BlockingState):
    ...
    def transition_function(self, userdata):
        # self.get_logger().info(f'{self.get_name()}: Executing state: {type(self).__name__}')
        print("EXECUTING STATE: ", type(self).__name__)
...
class SayImGoingTo(TaskER.BlockingState):
    ...
    def transition_function(self, userdata):
        self.get_logger().info(
            f'{self.get_name()}: Executing state: {type(self).__name__}')
        ...
        self.conversation_interface.speakNowBlocking(
            u'niekorzystne warunki pogodowe jadę do {"' + place_name + u'", dopelniacz}')
...
class SayIdontKnow(TaskER.BlockingState):
    ...
    def transition_function(self, userdata):
        ...
        self.conversation_interface.speakNowBlocking(
            u'niekorzystne warunki pogodowe nie wiem gdzie jest {"' + place_name + u'", mianownik}')
...
class SayIArrivedTo(TaskER.BlockingState):
    ...
    def transition_function(self, userdata):
        ...
        self.conversation_interface.speakNowBlocking(
            u'niekorzystne warunki pogodowe dojechałem do {"' + place_name + u'", dopelniacz}')
    ...
    def __init__(self, sim_mode, conversation_interface):
        ...
        TaskER.BlockingState.__init__(self,
                                      outcomes=['ok', 'preemption',
                                                'error', 'stall', 'shutdown'],
                                      input_keys=['move_goal', 'susp_data'])

        self.description = u'Jadę'
        self.suspendable_move_to = MoveTo(
            self.sim_mode, self.conversation_interface)
...
class MoveTo(TaskER.SuspendableState):
    def __init__(self, sim_mode, conversation_interface):
        ...
        self.conversation_interface = conversation_interface
        self.nav2_client = Nav2Client()
        self.clock = Nav2Client.get_clock()
    ...
    def transition_function(self, userdata):
        ...
        answer_id = self.conversation_interface.setAutomaticAnswer(
            'q_current_task', u'niekorzystne warunki pogodowe jadę do {"' + place_name + u'", dopelniacz}')
        ...
        if self.sim_mode == 'sim':
            ...
            goal.header.stamp = self.clock.now().to_msg()
            ...
            last_human_update = self.clock.now()
            self.is_goal_achieved = False
            while self.is_goal_achieved == False:
                ...
                loop_time_s = loop_time.nanoseconds * 1e-9

                if self.__shutdown__:
                    Nav2Client.cancel_all_goals()
                    self.conversation_interface.removeAutomaticAnswer(
                        answer_id)
                    self.service_preempt()
                    return 'shutdown'
                ...
                if self.update_destination_pose(userdata):
                    goal.pose = userdata.move_goal.parameters['pose']
                    # client.send_goal(goal, self.move_base_done_cb, self.move_base_active_cb, self.move_base_feedback_cb)
                    Nav2Client.send_goal(goal)
                # print "\n\n\n"
                # print "======================================"
            ...
            self.conversation_interface.removeAutomaticAnswer(answer_id)
            self.navigation_goal_status = Nav2Client.get_current_goal_status()
...
class MoveToHuman(MoveTo):
    def __init__(self, sim_mode, conversation_interface):
        ...
        self.Nav2Client = Nav2Client()
        self.clock = Nav2Client.get_clock()
        self.navigation_goal_status
        MoveTo.__init__(self, sim_mode, conversation_interface)

    def get_current_goal_status(self):
        self.navigation_goal_status = self.Nav2Client.get_current_goal_status()
        return self.navigation_goal_status
    ...
    def set_destination_pose(self, userdata):
        ...
        dest_pose.orientation = makePose(
            0, 0, human_pose[2]-math.pi).orientation
        userdata.move_goal.parameters['pose'] = dest_pose

    def update_destination_pose(self, userdata):
        if self.last_human_pose_update is None:
            self.last_human_pose_update = self.clock.time().now()
        if self.clock.time().now() < self.last_human_pose_update + rospy.Duration.from_sec(self.HUMAN_POSE_UPDATE_IN_APPROACH):
            return False
        self.last_human_pose_update = self.clock.time().now()
        ...
        dest_pose = PoseStamped()
        dest_pose.position.x = current_human_pose['x'] + \
            1*math.cos(current_human_pose['theta'])
        dest_pose.position.y = current_human_pose['y'] + \
            1*math.sin(current_human_pose['theta'])
        dest_pose.orientation = makePose(
            0, 0, current_human_pose['theta']-math.pi).orientation
        userdata.move_goal.parameters['pose'] = dest_pose
        return True
...
class MoveToAwareHazards(MoveTo):
    ...
    def transition_function(self, userdata):
        ...
        if self.sim_mode == 'sim':
            ...
            goal.pose = pose
            goal.header.frame_id = 'map'
            goal.header.stamp = self.clock.time().now().to_msg()
            ...
            if self.sim_mode not in ['sim', 'gazebo']:
                client_autonomous_head = actionlib.SimpleActionClient(
            ...
            client_move_head = actionlib.SimpleActionClient(
            ...
            start_time = clock.time().now()
            last_human_update = clock.time().now()
            self.is_goal_achieved = False
            self.hazard_trigger = False
            while ((self.is_goal_achieved == False or self.get_current_goal_status() == GoalStatus.STATUS_ABORTED) or self.hazard_trigger == True):
                # action_feedback.feedback.current_pose = self.current_pose

                # userdata.nav_feedback = action_feedback.feedback
                # userdata.nav_actual_pose = self.current_pose

                end_time = clock.time().now()
                loop_time = end_time - start_time
                ...
                if loop_time_s > NAVIGATION_MAX_TIME_S:
                    ...
                    print('State: Navigation took too much time, returning error')
                    Nav2Client.cancel_all_goals()
                    return 'stall'
                ...
                if self.hazard_trigger:
                    ...
                    print("UWAGA! ZNALAZLEM ", hazard_object,
                          " NA PODLODZE. PROSZE OMNIN TA PRZESZKODE")
                    # goal is interrupted by the hazard cancel goal
                    Nav2Client.cancel_goal()
                    answer_id = self.conversation_interface.speakNowBlocking(
                        u'niekorzystne warunki pogodowe Uwaga! Znalazłem {"' + hazard_object + u'", biernik} na podłodze. Proszę omiń tą przeszkodę.')
                    time.sleep(3)
            ...
            goal_status = self.get_current_goal_status()
            if (goal_status == GoalStatus.STATUS_SUCCEEDED):
                ...
                raise Exception(
                    'Wrong move_base action status: "' + str(goal_status) + '"')
...
class TurnAround(TaskER.BlockingState):
    ...
    def transition_function(self, userdata):
        ...
        if theta < 0:
            new_pose = makePose(
                pose.position.x, pose.position.y, theta+math.pi)
        else:
            new_pose = makePose(
                pose.position.x, pose.position.y, theta-math.pi)
        ...
        if self.sim_mode == 'sim':
            ...
            goal.pose = new_pose
            goal.header.frame_id = 'map'
            goal.header.stamp = clock.time().now().to_msg()
